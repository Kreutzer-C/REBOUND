...
class NLBlockND_multicross_block(nn.Module):
    """
    Non-Local Block for multi-cross attention (No-Parallel, need improve).
    ...
    def __init__(self, in_channels, inter_channels):
        ...
        self.W_q = nn.Conv2d(in_channels=self.in_channels, out_channels=self.inter_channels, kernel_size=1)
        self.W_k = nn.Conv2d(in_channels=self.in_channels, out_channels=self.inter_channels, kernel_size=1)
        self.W_v = nn.Conv2d(in_channels=self.in_channels, out_channels=self.inter_channels, kernel_size=1)

        self.W_o = nn.Sequential(
            nn.Conv2d(in_channels=self.inter_channels, out_channels=self.inter_channels, kernel_size=1),
    ...
    def forward(self, x_thisBranch, x_otherBranch):
        ...
        q_x = self.W_q(x_otherBranch).view(batch_size, self.inter_channels, -1).permute(0, 2, 1)
        k_x = self.W_k(x_thisBranch).view(batch_size, self.inter_channels, -1)       
        v_x = self.W_v(x_thisBranch).view(batch_size, self.inter_channels, -1).permute(0, 2, 1)
        
        f = torch.matmul(q_x, k_x)
        f_div_C = F.softmax(f / math.sqrt(self.inter_channels), dim=-1)
        
        y = torch.matmul(f_div_C, v_x)
        y = y.permute(0, 2, 1).contiguous().view(batch_size, self.inter_channels, *x_thisBranch.size()[2:])
...
class MultiHeadCrossAttention(nn.Module):
    def __init__(self, in_channels, num_heads):
        ...
        self.inter_channels = self.in_channels // self.num_heads
        
        self.cross_attention_heads = nn.ModuleList([
            NLBlockND_multicross_block(
    ...
    def forward(self, x_thisBranch, x_otherBranch):
        outputs = []
        for cross_attention in self.cross_attention_heads: # No-Parallel, need improve
