...
def parse_args():
    ...
    parser.add_argument('--model', '-m', type=str, default='CSANet',
                        help='Model name')
    parser.add_argument('--model_config', '-mc', type=str, default='./networks/R50_ViTB16_config.json',
...
def set_device(gpu_ids):
    ...
    if torch.cuda.is_available():
        ...
        devices = [int(gpu_id) for gpu_id in gpu_ids.split(',')]
        return torch.device(f'cuda:{devices[0]}') if len(devices) == 1 else [torch.device(f'cuda:{gpu_id}') for gpu_id in devices]
    else:
